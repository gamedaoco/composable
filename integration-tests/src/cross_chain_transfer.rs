...
fn transfer_from_picasso_to_dali() {
	...
	Dali::execute_with(|| {
		assert_ok!(<dali_runtime::AssetsRegistry as RemoteAssetRegistry>::set_location(
			...
			composable_traits::assets::XcmAssetLocation(MultiLocation::new(1, X2(Parachain(PICASSO_PARA_ID), GeneralKey(CurrencyId::PICA.encode()))))
		));
	});
	...
	Dali::execute_with(|| {
		let a = dali_runtime::Assets::free_balance(CurrencyId::PICA, &AccountId::from(BOB));
		let b = dali_runtime::Assets::free_balance(CurrencyId::INVALID, &AccountId::from(BOB));
		let c = dali_runtime::Assets::free_balance(CurrencyId::INVALID, &AccountId::from(ALICE));
