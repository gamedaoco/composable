...
proptest! {
	...
	new_test_ext().execute_with(|| {
		...
		let min_amt = vec![0u128, 0u128];
		assert_eq!(Tokens::balance(MockCurrencyId::USDC, &ConstantProductAmm::account_id(&pool_id)), alice_balance as u128 + bob_balance as u128);
		assert_eq!(Tokens::balance(MockCurrencyId::USDT, &ConstantProductAmm::account_id(&pool_id)), alice_balance as u128 + bob_balance as u128);
		...
		prop_assert_acceptable_computation_error!(Tokens::balance(MockCurrencyId::USDT, &ALICE), alice_balance as u128);
		prop_assert_acceptable_computation_error!(Tokens::balance(MockCurrencyId::USDC, &ALICE), alice_balance as u128);
		prop_assert_acceptable_computation_error!(Tokens::balance(MockCurrencyId::USDC, &ConstantProductAmm::account_id(&pool_id)), bob_balance as u128);
		prop_assert_acceptable_computation_error!(Tokens::balance(MockCurrencyId::USDT, &ConstantProductAmm::account_id(&pool_id)), bob_balance as u128);
		...
		prop_assert_zero_epsilon!(Tokens::balance(MockCurrencyId::USDC, &ConstantProductAmm::account_id(&pool_id)));
		prop_assert_zero_epsilon!(Tokens::balance(MockCurrencyId::USDT, &ConstantProductAmm::account_id(&pool_id)));
		prop_assert_acceptable_computation_error!(Tokens::balance(MockCurrencyId::USDT, &BOB), bob_balance as u128);
		prop_assert_acceptable_computation_error!(Tokens::balance(MockCurrencyId::USDC, &BOB), bob_balance as u128);
		Ok(())
	}).unwrap();
	}
	#[test]
	...
	new_test_ext().execute_with(|| {
		...
		assert_ne!(Tokens::balance(MockCurrencyId::USDC, &CHARLIE), 0);
	});
	}
}
