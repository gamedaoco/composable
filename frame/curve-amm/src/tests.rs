...
proptest! {
	...
	new_test_ext().execute_with(|| {
		...
		prop_assert_acceptable_computation_error!(Tokens::balance(MockCurrencyId::USDC, &CurveAmm::account_id(&pool_id)), bob_balance as u128);
		prop_assert_acceptable_computation_error!(Tokens::balance(MockCurrencyId::USDT, &CurveAmm::account_id(&pool_id)), bob_balance as u128);
